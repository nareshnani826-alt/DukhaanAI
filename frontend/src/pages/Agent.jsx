...
function needsUdhar(q) {
  return /udh|khata|due|baki|bakaya|baaki|கடன்|ఉధార్|उधार/i.test(q);
}
function needsWastage(q) {
  return /wast|expire|damage|nashan|waste/i.test(q);
}
...
export default function Agent() {
  ...
  async function send(text) {
    ...
    setMessages((prev) => [
      ...prev,
      { id: Date.now(), role: "user", text: q, time: timestamp() },
    ]);
    setLoading(true);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, role: "assistant", text: "⚠️ Connection error. Please try again.", time: timestamp() },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "var(--bg1, #f7f4f2)" }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0"
        style={{ background: "var(--saffron, #e87722)" }}>
        <div className="flex items-center justify-center rounded-full flex-shrink-0"
          style={{ width:40, height:40, background:"rgba(255,255,255,0.2)",
            border:"2px solid rgba(255,255,255,0.35)", fontSize:20 }}>
          🤖
        </div>
        ...
        <select value={lang} onChange={(e) => setLang(e.target.value)}
          className="text-xs rounded-full px-3 py-1 font-medium cursor-pointer border-0"
          style={{ background:"rgba(255,255,255,0.9)", color:"var(--saffron, #e87722)", outline:"none" }}>
      ...
      {/* Messages */}
      <div ref={msgsRef} className="flex-1 overflow-y-auto flex flex-col gap-2 px-3 py-3"
        ...
        {messages.map((msg) => (
          <div key={msg.id}
            ...
            {msg.role === "assistant" && (
              <div className="flex items-center justify-center rounded-full flex-shrink-0"
                style={{ width:30, height:30, background:"var(--saffron, #e87722)", fontSize:14 }}>
                🤖
              </div>
            )}
            <div style={{ maxWidth:"78%" }}>
              <div className="text-sm leading-relaxed whitespace-pre-wrap"
                style={{
                  ...
                  background:  msg.role === "user" ? "var(--saffron, #e87722)" : "var(--bg0, #ffffff)",
                  color:       msg.role === "user" ? "white" : "var(--ink, #1a1a1a)",
                  boxShadow:   msg.role === "user" ? "0 1px 4px rgba(232,135,34,0.25)" : "0 1px 4px rgba(0,0,0,0.07)",
                  border:      msg.role === "assistant" ? "0.5px solid var(--border, #e5ddd7)" : "none",
                }}>
                {msg.text}
              </div>
              <p className="text-xs mt-1"
                style={{ color:"var(--ink3, #bbb)", textAlign: msg.role === "user" ? "right" : "left" }}>
                {msg.time}
              </p>
            </div>
          </div>
        ...
        {loading && (
          <div className="flex items-end gap-2">
            ...
            <div className="flex items-center gap-1"
              style={{ padding:"12px 14px", borderRadius:"16px 16px 16px 4px",
                background:"var(--bg0, #ffffff)", border:"0.5px solid var(--border, #e5ddd7)",
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            {chips.map((chip) => (
              <button key={chip} onClick={() => send(chip)}
                ...
                style={{ background:"var(--bg0, white)", border:"1.5px solid var(--saffron, #e87722)",
                  color:"var(--saffron, #e87722)", cursor:"pointer", transition:"all 0.15s" }}
      ...
      <div className="flex items-center gap-2 px-3 py-2 flex-shrink-0"
        style={{ background:"var(--bg0, #ffffff)", borderTop:"0.5px solid var(--border, #e5ddd7)" }}>
        <div className="flex items-center flex-1 gap-2 rounded-full px-3"
          style={{ background:"var(--bg1, #f7f4f2)", border:"0.5px solid var(--border, #e5ddd7)", minHeight:40 }}>
          <input ref={inputRef} type="text" value={input}
            ...
            disabled={loading} />
          <button onClick={startMic} title="Voice input" aria-label="Voice input"
            style={{ background:"none", border:"none", color:"var(--saffron, #e87722)",
              cursor:"pointer", padding:"2px 0", display:"flex", alignItems:"center", flexShrink:0 }}>
        ...
        <button onClick={() => send()} disabled={loading || !input.trim()} aria-label="Send"
          style={{ width:40, height:40, borderRadius:"50%", background:"var(--saffron, #e87722)",
      ...
      <p className="text-center py-1 flex-shrink-0"
