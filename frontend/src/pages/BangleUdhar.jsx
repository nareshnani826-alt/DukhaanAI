...
// Saffron-brass palette for bangle store
const ACCENT   = "#e87722"
const ACCENT_BG = "rgba(232,119,34,0.10)"
const dueColor = due => due > 5000 ? "#E24B4A" : due > 1000 ? "#e87722" : "#1D9E75"
...
export default function BangleUdhar() {
  ...
  async function save() {
    ...
    if (!form.amount && modal!=="add-customer" && modal!=="edit-customer") return showNotif("Amount required")
    setSaving(true)
    try {
      if (modal === "add-customer") {
        ...
        showNotif("₹" + form.amount + " udhar added")
      ...
      if (selected) {
        ...
        if (fresh) { setSelected(fresh); setTxns(await Udhar.transactions(fresh.id)) }
      }
  ...
  async function deleteCustomer(c, e) {
    e?.stopPropagation()
    if (!confirm("Remove " + c.name + " from Khata? All transactions will be deleted.")) return
...
*${vendor?.store_name || "Humari Bangle Dukaan"}* se yaad dila rahe hain:
...
Store: ${vendor?.store_name || "DukaanAI Bangle Store"}
...
_Powered by DukaanAI_ 💍`
    ...
    win.document.write(`
      <!DOCTYPE html><html><head>
        ...
        <style>
          ...
          .big { font-size: 20px; font-weight: bold; color: ${txn.type==="credit"?"#e87722":"#1D9E75"}; }
        ...
        <div class="center">
          <div class="bold" style="font-size:16px">${vendor?.store_name || "DukaanAI Bangle Store"}</div>
        ...
        <div class="proof-box">
          ...
          ${txn.type==="credit"
            ? `<b>${c.name}</b> has taken bangles worth <b>${INR(txn.amount)}</b> on credit from <b>${vendor?.store_name||"this store"}</b>.`
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:20px">
          <div><div class="signature">Customer signature</div><div style="font-size:10px;color:#666">${c.name}</div></div>
          <div><div class="signature">Store signature</div><div style="font-size:10px;color:#666">${vendor?.store_name||"Store"}</div></div>
        </div>
        <div class="center" style="margin-top:16px">
          <button onclick="window.print()" style="background:#e87722;color:#fff;border:none;padding:8px 20px;border-radius:6px;cursor:pointer;font-size:13px">
            Print Receipt
          </button>
        </div>
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col p-4">
      {notif && (
        <div className="fixed top-4 right-4 text-white px-4 py-2 rounded-xl text-xs z-50 shadow-lg animate-fade-in"
        ...
        <div className="fixed inset-0 bg-black/40 z-40 flex items-center justify-center"
          ...
          <div className="bg-white rounded-2xl p-5 shadow-xl animate-slide-up" style={{ width:384, maxWidth:"95vw" }}>
              ...
              <div className="space-y-2.5">
                <div><label className="label">Name *</label>
                  <input className="input" value={form.name||""} onChange={e=>set("name",e.target.value)}
                    placeholder="Priya Sharma" autoFocus /></div>
                ...
                <div><label className="label">Address</label>
                  <input className="input" value={form.address||""} onChange={e=>set("address",e.target.value)}
                    placeholder="House or area" /></div>
              </div>
            ) : (
              <div className="space-y-2.5">
                {selected && (
                  <div className="px-3 py-2 rounded-xl text-xs font-medium"
                    style={{ background: modal==="credit" ? "rgba(232,119,34,0.10)" : "rgba(29,158,117,0.10)",
                             color:      modal==="credit" ? "#92400e"               : "#085041" }}>
                ...
                <div>
                  ...
                  <input className="input" value={form.note||""} onChange={e=>set("note",e.target.value)}
                    placeholder={modal==="credit" ? "e.g. Bangles, Kangan set" : "e.g. Cash payment"} />
                </div>
            ...
            <button onClick={save} disabled={saving}
              className="w-full py-2.5 text-xs font-semibold mt-4 rounded-xl border-none text-white cursor-pointer"
              style={{ background: modal==="credit" ? ACCENT : "#1D9E75", opacity: saving ? 0.7 : 1 }}>
              {saving ? "Saving..." :
               modal==="add-customer"  ? "Add Customer" :
      ...
      <div className="flex gap-3 flex-1 overflow-hidden min-h-0"
        ...
        {/* Left — customer list */}
        <div style={{ width: window.innerWidth <= 768 ? "100%" : "288px", flexShrink:0,
          display: window.innerWidth <= 768 && showDetail ? "none" : "flex", flexDirection:"column" }}>
            ...
            {loading ? (
              ...
              <div key={c.id}
                ...
                <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0"
                  style={{
                    background: c.total_due > 5000 ? "#FCEBEB" : c.total_due > 1000 ? ACCENT_BG : "#E1F5EE",
        ...
        <div style={{ flex:1, overflowY:"auto", minWidth:0,
          display: window.innerWidth <= 768 && !showDetail ? "none" : "block" }}>
            ...
            <div className="space-y-3">
              ...
              <div className="card">
                <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
                  ...
                  <div className="text-right">
                    ...
                    {selected.total_due === 0 && (
                      <div className="text-[10px] font-medium" style={{ color: ACCENT }}>✓ Account clear</div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mt-2">
                  <button onClick={() => openModal("edit-customer", selected)}
                    className="text-[10px] underline" style={{ color: ACCENT, background:"none", border:"none", cursor:"pointer" }}>
                    ✏ Edit details
                  </button>
              ...
              <div className="card">
                ...
                {txns.length === 0 ? (
                  ...
                  <div key={txn.id}
                    ...
                    style={{ background: txn.type==="credit" ? ACCENT_BG : "rgba(29,158,117,0.08)" }}>
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0"
                      style={{
                        background: txn.type==="credit" ? "rgba(232,119,34,0.20)" : "#E1F5EE",
                        color:      txn.type==="credit" ? "#92400e"               : "#085041",
                      }}>
                      {txn.type === "credit" ? "📤" : "📥"}
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="text-right flex-shrink-0">
                      <div className="text-sm font-bold"
                        style={{ color: txn.type==="credit" ? ACCENT : "#1D9E75" }}>
                        {txn.type === "credit" ? "+" : "-"}{INR(txn.amount)}
                      </div>
                      <div className="flex gap-1 mt-1 justify-end">
                        {selected.phone && (
                          <button onClick={()=>sendProof(selected, txn)}
                            className="text-[9px] px-1.5 py-0.5 rounded font-medium"
                            style={{ background:"#25D366", color:"#fff", border:"none", cursor:"pointer" }}>
                            WA Proof
                          </button>
                        )}
                        <button onClick={()=>printProof(selected, txn)}
